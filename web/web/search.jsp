...
<div class="contain">
    ...
    <div style="margin-top: 20px">
        <table class="table table-hover tb">
            ...
            <tbody>
            <%
                String pa=request.getParameter("search");
                String se="%"+ pa+"%";
                se=se.replace("'","");
                String sqlQuery = "SELECT * FROM studentinfo WHERE studentId like '"+se+"' OR name like '"+se+"' or major like '"+se+"' OR classname like '"+se+"' "+"limit "+  startLine + "," + pageSize;//对数据库进行伪分页读取，一次只能读10行
//                out.print(sqlQuery);
                String sqlAll = " SELECT * FROM studentinfo WHERE studentId like '"+se+"' OR name like '"+se+"' or major like '"+se+"' OR classname like '"+se+"'";
        ...
        <div class="pageNav">
            <ul class="pagination">
                ...
                %>
                <li class="page-item"><a class="page-link" href="frame.jsp?type=search&paramets=<%=pa%>&pages=<%=prePage%>">上一页</a></li>
                <%
                    ...
                    for (int i = minpages; i <= maxpages; i++) {
                        if (i == pagesNo) {//当前页和遍历出来的页数相等时，需要通过调用css里面的样式“active"进行高亮
                            ...
                            String hr="frame.jsp?type=search&paramets="+pa+"&pages="+i;
                            out.print("<a class='page-link' href="+hr + ">" + i + "</a>");
                            out.print("</li>");
                        } else {//输出每一个分页
                ...
                %>
                <li class="page-item"><a class="page-link" href="frame.jsp?type=search&paramets=<%=pa%>&pages=<%=nextPage%>">下一页</a></li>
            </ul>
        </div>
